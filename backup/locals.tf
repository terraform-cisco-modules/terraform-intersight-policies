locals {
  ...
  pin_groups = { for i in flatten([for k, v in local.port : [for e in v.pin_groups : merge(e, {
    breakout_port_id = e.ilength == 3 ? element(split("/", e.identifier), 1) : 0
    ...
    pc_id = e.interface_type == "port_channel" ? e.identifier : 0
    ...
    port_id = e.ilength == 3 ? element(split("/", e.identifier), 2) : e.ilength == 2 ? element(split("/", e.identifier), 1) : 0
    slot_id = e.interface_type == "port" ? element(split("/", e.identifier), 0) : 1
    tags    = v.tags
  })]]) : "${i.port_policy}/${i.pin_group_type}/${i.pc_id}/${i.slot_id}-${i.breakout_port_id}-${i.port_id}" => i }
  port_pin_groups = { for k, v in local.pin_groups : k => merge(v, {
  ...
  vhba_template = { for i in flatten([for org in sort(keys(var.model)) : [
    for v in lookup(lookup(var.model[org], "policies", {}), "vhba_template", []) : merge(local.vhba_templates, v, {
  ...
  ] if length(lookup(lookup(var.model[org], "policies", {}), "vhba_template", [])) > 0]) : "${i.organization}/${i.key}" => i }

...
resource "intersight_vnic_eth_if" "map" {
  ...
  eth_adapter_policy {
    moid = lookup(local.ethernet_adapter, each.value.ethernet_adapter_policy.name, "#NOEXIST"
      ) != "#NOEXIST" ? intersight_vnic_eth_adapter_policy.map[each.value.ethernet_adapter_policy.name
  ...
  eth_qos_policy {
    moid = lookup(local.ethernet_qos, each.value.ethernet_qos_policy.name, "#NOEXIST"
      ) != "#NOEXIST" ? intersight_vnic_eth_adapter_policy.map[each.value.ethernet_qos_policy.name
    ].moid : intersight_vnic_eth_adapter_policy.data["${each.value.ethernet_qos_policy.org}:${each.value.ethernet_qos_policy.name}"].moid
  }
  ...
  usnic_settings {
    ...
    usnic_adapter_policy = length(regexall("UNUSED", each.value.usnic_settings.usnic_adapter_policy.name)
      ) == 0 ? lookup(each.value.usnic_settings, each.value.usnic_settings.usnic_adapter_policy.name, "#NOEXIST"
      ) != "#NOEXIST" ? intersight_vnic_eth_adapter_policy.map[each.value.usnic_settings.usnic_adapter_policy.name
    ].moid : intersight_vnic_eth_adapter_policy.data["${each.value.usnic_settings.usnic_adapter_policy.org}:${each.value.usnic_settings.usnic_adapter_policy.name}"].moid : ""
  }
  vmq_settings {
    ...
    vmmq_adapter_policy = length(regexall("UNUSED", each.value.usnic_settings.vmmq_adapter_policy.name)
      ) == 0 ? lookup(each.value.vmq_settings, each.value.vmq_settings.vmmq_adapter_policy.name, "#NOEXIST"
      ) != "#NOEXIST" ? intersight_vnic_eth_adapter_policy.map[each.value.vmq_settings.vmmq_adapter_policy.name
    ].moid : intersight_vnic_eth_adapter_policy.data["${each.value.vmq_settings.vmmq_adapter_policy.org}:${each.value.vmq_settings.vmmq_adapter_policy.name}"].moid : ""
  }
  dynamic "eth_network_policy" {
    for_each = { for v in [each.value.ethernet_network_policy] : v.name => v if v.name != "UNUSED" }
    content {
      moid = lookup(local.ethernet_network, eth_network_policy.value.name, "#NOEXIST"
        ) != "#NOEXIST" ? intersight_vnic_eth_network_policy.map[eth_network_policy.value.name
  ...
  dynamic "fabric_eth_network_group_policy" {
    ...
    content {
      moid = lookup(local.ethernet_network_group, fabric_eth_network_group_policy.value.name, "#NOEXIST"
        ) != "#NOEXIST" ? intersight_fabric_eth_network_group_policy.map[fabric_eth_network_group_policy.value.name
  ...
  dynamic "iscsi_boot_policy" {
    ...
    content {
      moid = lookup(local.iscsi_boot, iscsi_boot_policy.value.name, "#NOEXIST"
        ) != "#NOEXIST" ? intersight_vnic_iscsi_boot_policy.map[iscsi_boot_policy.value.name
